...
- [ ] Separately reported (not reproducible from git, needs live dev server access): tsx/vite watcher showing 3 stale TypeScript errors (toggleOwnerJoined, getAdminOpportunityView, ownerJoined in ClickNotification) that don't appear in a clean npx tsc --noEmit (confirmed 0 errors locally against current main). All three symbols exist in the current code (dbCommunities.ts, drizzle/schema.ts). This is a dev-server language-service cache issue, not a code bug — restart the watcher/dev server.
