---
title: "مئة يوم من الكتابة Review 2026: A Free Arabic Writing Community"
meta_description: "مئة يوم من الكتابة review: a free, 5,317-member Arabic-language Skool community helping writers build a 100-day writing habit."
slug: 100-days-of-writing-5075
niche: "Arabic-language writing habit and craft community"
word_count: 0
---

مئة يوم من الكتابة translates to "100 Days of Writing," and that's exactly the premise of this free, 5,317-member Arabic-language Skool community: write consistently for one hundred days and leave your hesitation behind.

## What's Actually Inside

The description states, in Arabic, that over the next hundred days members will write together and let go of their hesitation, describing the community's purpose as preparing writers to master the craft of writing and commit to it consistently. That's a structure built around habit formation over a fixed period, rather than a one-time course or a loose discussion group. The description doesn't specify whether there are daily prompts, accountability check-ins, or peer feedback built into the format, so the exact mechanics of the "100 days" structure aren't confirmed here.

## Who Runs It

محمد الضبع (Mohammed Aldhabaa) is listed as the owner of this community on Skool. No further independent background or credentials are available in the data reviewed here.

## Pros

- Free to join, removing any financial barrier for aspiring writers.
- A clear, time-bound structure, 100 days, that gives members a concrete commitment period rather than an open-ended promise.
- Framed around overcoming hesitation, which speaks directly to a common blocker for new writers.

## Cons

- The description doesn't detail the actual daily or weekly mechanics, prompts, feedback, or accountability partners, so members won't know the format until joining.
- Non-Arabic speakers won't be able to participate meaningfully, since the description and presumably the community itself operate entirely in Arabic.
- No independent reviews of this community were found beyond its own Skool listing.

## Who It's a Good Fit For

Arabic-speaking aspiring writers who want a structured, time-bound push to build a consistent writing habit and are motivated by a defined 100-day commitment.

## Who Should Skip It

Non-Arabic speakers obviously won't be able to use this community, and writers looking for detailed craft instruction, specific genres, editing feedback, or publishing guidance, rather than habit-building accountability, should confirm the format covers that before joining.

[See مئة يوم من الكتابة's real growth data on TrustSkool](https://trustskool.com/community/100-days-of-writing-5075)
